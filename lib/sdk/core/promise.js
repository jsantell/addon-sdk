...
function fulfilled(value) {
  return {
    then: function then(fulfill) { fulfill(value); },
    done: function done(fulfill) { fulfill(value); }
  };
}
...
function rejected(reason) {
  return {
    then: function then(fulfill, reject) { reject(reason); },
    done: function done(fulfill, reject) { reject(reason); }
  };
}
...
function defer(prototype) {
  ...
  var promise = Object.create(prototype, {
    ...
    done: { value: function (onFulfill, onError) {
      onFulfill = onFulfill || function (v) {
        console.log('ONFULFILL CALLEEDDLKJFL');
      };
      
//      var result = onFulfill ? onFulfill(value) : value;


      onError = onError || function (reason) {
        console.log('****', reason);
        throw reason;
      };
      if (observers) {
        observers.push({ resolve: onFulfill, reject: onError });
      }
      // Otherwise just forward observer pair right to a `result` promise.
      else {
        result.then(onFulfill, onError);
      }
    }}
  })
    ...
    resolve: function resolve(value) {
      if (!result) {
        ...
        while (observers.length) {
          ...
          result.done(observer.resolve, observer.reject);
        }
